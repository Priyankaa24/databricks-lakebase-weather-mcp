...
Tracing & Monitoring:
    ...
    (mcp_tool_traces_weather) with session ID, user email, parameters, results,
...
def _ensure_traces_table():
    ...
    try:
        ...
        logger.warning(f"Could not ensure mcp_tool_traces_weather table (may already exist): {e}")
...
def _log_tool_call_to_lakebase(
    ...
    try:
        ...
        lakebase.run_write(
            """
            INSERT INTO mcp_tool_traces_weather
...
@mcp.tool
@traced_tool
def get_recommendation(location: str, date_offset: int = 0) -> dict:
    """
    Get practical weather-based recommendations for a location and date.

    Use this when the user asks for advice or a judgment call rather than
    raw data - "should I bring an umbrella?", "do I need a jacket?", "is
    it a good day for a run?", "safe to drive?". This tool reasons over
    the forecast (precipitation chance/amount, temperatures, wind,
    conditions) and returns yes/no recommendations WITH explanations for:

    - umbrella: rain gear needed?
    - jacket: warm layers needed?
    - sunscreen: UV protection needed?
    - outdoor_activities: good day for running/cycling/picnic?
    - travel: safe driving conditions?

    Each recommendation includes a `reasoning` string the agent can quote
    directly to the user. Rules encoded here (thresholds like "rain chance
    over 40% = umbrella") come from typical guidance, not from the LLM
    guessing.

    Args:
        location: A place name, e.g. "Chicago, IL".
        date_offset: Days from today. 0 = today, 1 = tomorrow, up to 6.

    Returns:
        A dict with status, location, date, forecast_summary (the raw
        numbers used), and a 'recommendations' object containing umbrella,
        jacket, sunscreen, outdoor_activities, and travel - each with a
        boolean and a reasoning string.
    """
    try:
        result = weather_broker.get_recommendation(location, date_offset)
        return {"status": "success", **result}
    except Exception as e:
        logger.exception(f"get_recommendation failed for {location!r}")
        return {"status": "error", "message": f"Failed to build recommendation: {str(e)}"}
